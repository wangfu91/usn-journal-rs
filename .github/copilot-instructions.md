...
- `src\raw_mft\` reads the `$MFT` file directly from the volume and parses each FILE record into a rich `RawMftEntry` (full timestamps, real / allocated size, hard link count, alternate data streams, sparse / compressed / encrypted flags, data-run summary, file-name namespace). Submodules: `ondisk\` (`boot`, `fixup`, `attribute`, `data_run`, `record`, `extent` low-level parsers), `io` (sector-aligned `VolumeReader`), `bootstrap\` (`RawMft::new` setup plus FILE record 0 stream discovery), `entry_build\` (`entry`, `batch`, `fold`, `capture`, `names` for entry materialization), `serial\` (`iter`, `profile`, `engine` for the shared serial scan loop), `parallel\` (`scan`, `chunks`, `executor` for `RawMft::parallel()` and ordered parallel chunk execution), `chunk_plan` (`RawMftChunkPlanOptions`, `RawMftWorkChunk`), and `options` (`RawMftScanOptions`). `$ATTRIBUTE_LIST` is fully handled: resident lists are parsed inline; non-resident lists are read from disk via their data runs. Extension records are loaded and the highest-scoring file-name namespace wins (Win32AndDos > Win32 > Posix > Dos), eliminating spurious DOS 8.3 short names. **NTFS only** — ReFS volumes return `UsnError::UnsupportedFilesystem`.
